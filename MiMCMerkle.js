...
module.exports = {
    ...
    getZeroCache: function (zeroLeafHash, depth) {
        ...
        for (var i = 1; i < depth; i++) {
            zeroCache[i] = mimcjs.mimc([zeroCache[i - 1], zeroCache[i - 1]]);
        }
        return zeroCache;
    },
        ...
        if (merkle_path.length > 0) {
            ...
            left =
                ...
                bigInt(merkle_path_pos[0]) *
                    (bigInt(leaf) - bigInt(merkle_path[0]));
            right =
                bigInt(merkle_path[0]) -
                bigInt(merkle_path_pos[0]) *
                    (bigInt(merkle_path[0]) - bigInt(leaf));
            root[0] = mimcjs.mimc([left, right]);
            var i;
            for (i = 1; i < depth; i++) {
                left =
                    root[i - 1] -
                    bigInt(merkle_path_pos[i]) *
                        (root[i - 1] - bigInt(merkle_path[i]));
                right =
                    bigInt(merkle_path[i]) -
                    bigInt(merkle_path_pos[i]) *
                        (bigInt(merkle_path[i]) - root[i - 1]);
                root[i] = mimcjs.mimc([left, right]);
            }
    ...
    padLeafArray: function (leafArray, zeroLeaf, fillerLength) {
        if (Array.isArray(leafArray)) {
            ...
            const diff =
                fillerLength || 2 ** nearestPowerOfTwo - leafArray.length;
    ...
    pairwiseHash: function (array) {
        if (array.length % 2 == 0) {
            ...
            for (i = 0; i < array.length; i = i + 2) {
                arrayHash.push(
                    mimcjs.mimc([array[i].toString(), array[i + 1].toString()])
                );
            }
            return arrayHash;
        } else {
